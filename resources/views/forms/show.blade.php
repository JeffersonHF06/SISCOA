...
<div class="container text-style">
    ...
    <h5>Fecha y horario: {{$form->date->isoFormat('LL')}} / {{ $form->start_time->format('g:i A') }} - {{ $form->end_time->format('g:i A') }}</h5>
    ...
    <form action="/forms/addUserToForm/{{$form->id}}" id="search-form" method="POST">
       ...
       <div class="row-md">
           <div class="col my-3 d-flex justify-content-end">
            <button type="submit" class="btn btn-lg btn-danger"><i class="fas fa-sign-in-alt"></i> Registrarse</button>
           </div>
